...
uint8_t Ag::ogren::sinyal_gucu()
{
    ...
    {
        Serial.println(F("[AG] Wi-Fi baglantisi yok. Sinyal gücü ölçülemiyor."));
        return 0;
    }

    int rssi = WiFi.RSSI();
    Serial.print(F("[AG] RSSI Degeri: "));
    Serial.print(rssi);
    Serial.println(F(" dBm"));

    if (rssi >= -50)
    {
        Serial.println(F("[AG] Sinyal Seviyesi: Çok Güçlü (3/3)"));
        return 3;
    }
    else if (rssi >= -65)
    {
        Serial.println(F("[AG] Sinyal Seviyesi: Orta (2/3)"));
        return 2;
    }
    else if (rssi >= -75)
    {
        Serial.println(F("[AG] Sinyal Seviyesi: Zayıf (1/3)"));
        return 1;
    }
    else
    {
        Serial.println(F("[AG] Sinyal Seviyesi: Çok Zayıf (0/3)"));
        return 0;
    }
}
...
void Ag::ogren::durum()
{
    ...
    {
    case WL_CONNECTED:
        Serial.println(F("[AG] Wi-Fi baglantisi basarili."));
        break;
    case WL_NO_SSID_AVAIL:
        Serial.println(F("[AG] HATA: Belirtilen SSID bulunamadi."));
        break;
    case WL_CONNECT_FAILED:
        Serial.println(F("[AG] HATA: SSID veya sifre hatali."));
        break;
    case WL_IDLE_STATUS:
        Serial.println(F("[AG] Bilgi: Baglanti beklemede (IDLE)."));
        break;
    case WL_DISCONNECTED:
        Serial.println(F("[AG] Bilgi: Wi-Fi baglantisi kesildi."));
        break;
    case WL_SCAN_COMPLETED:
        Serial.println(F("[AG] Bilgi: Tarama tamamlandi."));
        break;
    default:
        Serial.println(F("[AG] Bilinmeyen Wi-Fi durumu."));
        break;
    }
}
...
String Ag::ogren::ip_adresi()
{
    if (WiFi.status() == WL_CONNECTED)
    {
        ...
        Serial.print(F("[AG] IP Adresi: "));
        Serial.println(ipStr);
        return ipStr;
    }
    else
    {
        Serial.println(F("[AG] IP adresi alinamadi. Wi-Fi baglantisi yok."));
        return "";
    }
}
...
String Ag::ogren::ag_adi()
{
    if (WiFi.status() == WL_CONNECTED)
    {
        ...
        Serial.print(F("[AG] Bagli olunan ag: "));
        Serial.println(ssid);
        return ssid;
    }
    else
    {
        Serial.println(F("[AG] Wi-Fi baglantisi yok. SSID alinamadi."));
        return "";
    }
}
...
{
    Serial.println(F("[AG] Ağlar taranıyor..."));
    int n = WiFi.scanNetworks();
    if (n <= 0)
    {
        Serial.println(F("[AG] Hiçbir ağ bulunamadı."));
        return F("[AG] Hiçbir ağ bulunamadı.\n");
    }
    int indices[n];
    for (int i = 0; i < n; ++i)
        indices[i] = i;
